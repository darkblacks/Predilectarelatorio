...
import { PRODUCTIVITY_DATA_LABEL, useProductivityWorkbook } from './slides/useProductivityWorkbook';
...
function Loading() {
  return (
    <div className="loading-screen">
      ...
      <span>
        Carregando {PRODUCTIVITY_DATA_LABEL}...
      </span>
    </div>
  );
}
  ...
  const slides = useMemo(
    () => [
      ...
      <SlideProdutividadeUnidades
        key="produtividade-unidades"
        units={units}
      />,

      <SlideMatrizProdutividade
        key="matriz"
        units={units}
      />,
  ...
  useEffect(() => {
    const handleKey = (event: KeyboardEvent) => {
      if (
        ...
        setIndex((current) =>
          Math.min(labels.length - 1, current + 1)
        );
      }
