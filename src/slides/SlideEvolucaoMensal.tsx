...
export function SlideEvolucaoMensal({ may, june, july }: SlideEvolucaoMensalProps) {
  ...
  const monthLabels = [may.label, june.label, july.label];
    ...
    series: [
      {
        ...
        custom: { recalculatePercent: true, rawValues: [mayOwn, juneOwn, julyOwn] },
      ...
      },
      {
        ...
        custom: { recalculatePercent: true, rawValues: [mayThirdParty, juneThirdParty, julyThirdParty] },
      ...
      },
      {
        ...
        custom: { recalculatePercent: true, rawValues: [may.fob, june.fob, july.fob] },
  ...
  return (
    <SlideWrapper
      ...
      subtitle={`${may.label} → ${june.label} → ${july.label}, acompanhando demanda, produção própria e Terceiros.`}
      footer={`Dashboard Operacional Predilecta · ${july.label}`}
    >
      <div className="story-page">
            ...
            {cards.map((card) => (
              ...
              >
                ...
                <div style={{ display: 'grid', gap: 5, marginTop: 10 }}>
                  ...
                  >
                    {may.label} → {june.label}: {signedPct(card.mayJune)}
                  </span>
                  <span
                    style={{
                  ...
                  >
                    {june.label} → {july.label}: {signedPct(card.juneJuly)}
                  </span>
                </div>
              </div>
            ))}
        ...
        <motion.section className="story-section" {...reveal}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 18 }}>
            ...
            >
              <span className="pill">{may.label} → {june.label}</span>
            ...
            >
              <span className="pill">{june.label} → {july.label}</span>
              <strong style={{ display: 'block', color: ink, fontSize: 25, marginTop: 14, letterSpacing: '-0.04em' }}>
                {july.label} atualiza a leitura da demanda e da produção própria.
              </strong>
              <p style={{ color: muted, margin: '10px 0 0', fontWeight: 740, lineHeight: 1.48 }}>
        ...
        <motion.section className="story-section" {...reveal}>
          <div className="charts-grid charts-grid--two">
            <ChartPanel
              title="Volume mensal"
              subtitle={`Demanda total, Próprio e Terceiro de ${may.label} a ${july.label}.`}
              option={optionVolumes}
              height={370}
            />
            <ChartPanel
              title="Participação na demanda"
              subtitle="Evolução percentual de Próprio, Terceiro e FOB. A legenda recalcula a base visível."
              option={optionShares}
              height={370}
            />
          </div>
