...
export function SlidePergunta({ meta, june, july, units }: SlidePerguntaProps) {
  ...
  const juneOperationalTotal = juneOwn + juneThirdParty;
  const julyOperationalTotal = julyOwn + julyThirdParty;

  const juneOwnShare = safeShare(juneOwn, juneOperationalTotal);
  const juneThirdPartyShare = safeShare(juneThirdParty, juneOperationalTotal);
  const juneFobShare = safeShare(june.fob, june.total);

  const julyOwnShare = safeShare(julyOwn, julyOperationalTotal);
  const julyThirdPartyShare = safeShare(julyThirdParty, julyOperationalTotal);
    ...
    series: [
      {
        ...
        custom: { recalculatePercent: true, rawValues: [julyOwn] },
      ...
      },
      {
        ...
        custom: { recalculatePercent: true, rawValues: [julyThirdParty] },
      ...
      },
      {
        ...
        custom: { recalculatePercent: true, rawValues: [july.fob] },
    ...
    series: [
      {
        ...
        custom: { recalculatePercent: true, rawValues: unitRows.map((item) => item.own) },
        itemStyle: { color: blue },
      },
      {
        name: 'Terceiro',
        ...
        custom: { recalculatePercent: true, rawValues: unitRows.map((item) => item.thirdParty) },
        itemStyle: { color: red },
      },
      {
        name: 'FOB',
        ...
        custom: { recalculatePercent: true, rawValues: unitRows.map((item) => item.fob) },
    ...
    series: [
      {
        ...
        type: 'bar',
        stack: 'total',
        data: unitRows.map((item) => item.own),
        itemStyle: { color: blue },
      },
      {
        name: 'Terceiro',
        type: 'bar',
        stack: 'total',
        data: unitRows.map((item) => item.thirdParty),
        itemStyle: { color: red },
      },
      {
        name: 'FOB',
    ...
    <SlideWrapper
      ...
      subtitle={
        view === 'produtividade'
          ...
          : `Resultado de ${july.label} pela ótica tradicional da meta de terceiros.`
      }
    >
          ...
          >
            ...
            >
              ...
              <ToggleButton active={view === 'resultado'} onClick={() => setView('resultado')}>
                Resultado tradicional · {july.label}
              </ToggleButton>
            </div>
          </div>
        </motion.section>
          ...
          <>
            <motion.section className="story-section" {...reveal}>
              <div className="story-section__heading">
                ...
                <h2>Big numbers · {july.label}</h2>
                <p>
                  Resultado consolidado de {july.label} apresentado em Próprio, Terceiro e FOB.
                </p>
              </div>
            ...
            <motion.section className="story-section" {...reveal}>
              <div className="story-section__heading">
                ...
                <p>
                  Composição consolidada de {july.label} entre Próprio, Terceiro e FOB.
                </p>
              </div>

              <div className="charts-grid charts-grid--two">
                <ChartPanel
                  title={`Participação em ${july.label} (%)`}
                  subtitle={`Percentual sobre o total de viagens de ${july.label}.`}
                  option={optionGrupoPercentual}
                  height={360}
                />
                <ChartPanel
                  title={`Quantidade de viagens · ${july.label}`}
            ...
            <motion.section className="story-section" {...reveal}>
              <div className="story-section__heading">
                ...
                <h2>Resultado atual de {july.label}</h2>
            ...
            <motion.section className="story-section" {...reveal}>
              <div className="story-section__heading">
                ...
                <h2>Comparativo por fábrica · {july.label}</h2>
              ...
              <div className="charts-grid charts-grid--two">
                ...
                <ChartPanel
                  ...
                  subtitle={`Volume absoluto de ${july.label}.`}
