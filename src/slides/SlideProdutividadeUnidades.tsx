...
interface SlideProdutividadeUnidadesProps {
  units: UnitSummary[];
}
...
export function SlideProdutividadeUnidades({ units }: SlideProdutividadeUnidadesProps) {
  ...
  const option = {
    ...
    yAxis: {
      type: 'category',
      data: ordered.map((item) => item.unit),
  ...
  return (
    <SlideWrapper
      eyebrow="Produtividade observada"
      title="Quanto cada fábrica está extraindo de seus veículos?"
      subtitle={`Viagens próprias ÷ quantidade de veículos informada na base de ${CURRENT_MONTH_LABEL}.`}
    >
        ...
        <motion.section className="story-section" {...reveal}>
          <div className="charts-grid charts-grid--two" style={{ alignItems: 'start' }}>
            <ChartPanel
              ...
              subtitle="Comparação da produção observada entre unidades."
              option={option}
              height={420}
            />
            ...
            <div
              style={{
                ...
                borderRadius: 30,
                padding: 20,
                boxShadow: '0 18px 56px rgba(129,0,27,.09)',
              }}
            >
              <div
                style={{
                  display: 'grid',
                  gridTemplateColumns: '1.25fr .7fr .7fr .85fr .85fr',
                  gap: 8,
                  padding: '10px 11px',
                  background: soft,
                  borderRadius: 14,
              ...
              <div style={{ display: 'grid', gap: 7, marginTop: 8 }}>
                {ordered.map((item) => (
                  ...
                  >
                    ...
                    <span style={{ color: muted, fontWeight: 850 }}>{item.own}</span>
                    ...
                    <strong style={{ color: item.thirdPartyShare >= 0.38 ? red : muted }}>
                      {pct(item.thirdPartyShare)}
                    </strong>
                  </div>
                ))}
              </div>
